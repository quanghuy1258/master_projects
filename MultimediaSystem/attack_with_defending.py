...
class WrapperModel():
    """
    Wraper model to hide target model.

    :param model_fn: Function that builds target model
    :param out_prop: Proportion of data outside training set
    :param num_param_sets: Number of parameter sets
    :param epsilon: Epsilon of differential privacy
    """
    def __init__(self, model_fn, out_prop, num_param_sets, epsilon):
        self.model = model_fn()
        self.wrapper = LogisticRegression(fit_intercept=False,
                                          multi_class="multinomial")
        self.out_prop = out_prop
    ...
    def fit(self, X, y, **fit_kwargs):
        ...
        :param X: Input training data of target model
        :param y: Output training data of target model
        :param fit_kwargs: Arguments that will be passed to the fit call for
                target model.
        """
        X_in, _, y_in, _ = train_test_split(X, y, test_size=self.out_prop)

        fit_kwargs = fit_kwargs or {}
        self.model.fit(X_in, y_in, **fit_kwargs)

        y_hat_in = self.model.predict(X_in)
        new_y_in = np.argmax(y_in, 1)
        self.wrapper.fit(y_hat_in, new_y_in)

        param_sets = []
        data_indices = np.arange(X.shape[0])
        y_hat = self.model.predict(X)
        new_y = np.argmax(y, 1)
        for i in tqdm(range(self.num_param_sets)):
            indices = data_indices[np.random.choice(self.num_param_sets,
                                                    X.shape[0],
                                                    replace=True) == i]
            wrapper = LogisticRegression(fit_intercept=False,
                                         multi_class="multinomial").fit(y_hat[indices],
                                                                        new_y[indices])
            param_sets.append(wrapper.coef_)

        self.sensitive = 0
        for i, j in itertools.product(range(self.num_param_sets),
                                      range(self.num_param_sets)):
            dist = np.linalg.norm((param_sets[i] - param_sets[j]).reshape(-1), ord=1)
            self.sensitive = max(self.sensitive, dist)

    def predict(self, X):
        result = self.model.predict(X)

        coef = self.wrapper.coef_
        for i in range(result.shape[0]):
            self.wrapper.coef_ = coef + np.random.laplace(scale=self.sensitive/self.epsilon,
                                                          size=coef.size).reshape(coef.shape)
            result[[i], :] = self.wrapper.predict_proba(result[[i], :])
        self.wrapper.coef_ = coef
...
def new_target_model_fn():
    return WrapperModel(target_model_fn,
                        out_prop=OUT_PROP,
